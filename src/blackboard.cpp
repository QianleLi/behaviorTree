...
void Blackboard::setPortInfo(std::string key, const PortInfo& info)
{
    ...
    else
    {
        ...
        {
            std::cout << "Blackboard::set() failed: once declared, the type of a port shall not "
                         "change. "
                      << "Declared type [" << BT::demangle(old_type) << "] != current type ["
                      << BT::demangle(info.type()) << "]";
            throw LogicError("Blackboard::set() failed: once declared, the type of a port shall "
                             "not change. "
                             "Declared type [",
